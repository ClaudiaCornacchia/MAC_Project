package com.example.mobile_app.presentation.box.box_detail
...
import com.example.mobile_app.data.repository.LocationService
...
import com.example.mobile_app.data.repository.AutocompleteResult
...
class BoxDetailViewModel @Inject constructor(
    ...
    fun updateLocation() {
        launchCatching {
            ...
            if (location != null && currentBox != null) {
                ...
                storageRepository.updateBoxFields(
    ...
    fun onShareConfirm(email: String) {
        ...
        launchCatching {
            if (email.isNotBlank()) {
                storageRepository.shareBoxWithUser(box.boxId, email)
            } else {
    ...
    private fun loadSharedNames() {
        launchCatching {
            val names = box.sharedWith
                ...
                .filter { it != accountRepository.currentUserId }
                .map { memberId ->
                    async { accountRepository.getUserName(memberId) }
                }
    ...
    fun saveEditChanges() {
        launchCatching {
            ...
            storageRepository.updateBoxFast(
    ...
    fun deleteBox(onSuccess: () -> Unit) {
        launchCatching {
            ...
            storageRepository.deleteBox(boxId, box.ownerId)
            isLoading = false
