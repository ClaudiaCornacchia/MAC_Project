...
import com.example.mobile_app.data.SpeechService
import com.example.mobile_app.data.SpeechState
...
class NewBoxViewModel @Inject constructor(
    ...
    fun onDoneClick(navigate: (String) -> Unit) {
        launchCatching {
            val newId = storageRepository.getNewBoxId()
            val newBox = Box(
                boxId = newId,
                title = uiState.title,
            ...
            storageRepository.saveBox(newBox, uiState.selectedImageUri)
